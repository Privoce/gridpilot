...
def _fill_attachment_a_xlsm(intake: dict, d: dict, eng: dict, path: Path) -> None:
    ...
    # PF regulation range (II.15/16) comes straight from the OEM datasheet
    # (Sungrow: adjustable 0.8 leading - 0.8 lagging). Never-hallucinate:
    # written only when the OEM PF range is sourced; otherwise the cells stay
    # blank and surface as SYSTEM ADMIN alerts.
...
def _gen_attachment_a_xlsx(intake: dict, d: dict, eng: dict, path: Path) -> None:
    ...
    # OEM adjustable PF range straight from the datasheet (e.g. Sungrow 0.8).
    _pv_pf = -float(inv["pf_range"]) if lib.is_sourced(inv, "pf_range") else ""
    _b_pf = (float(bess["pf_range"])
